...
def get_data(self, url, lang):
    ...
    for day_div in sel_day_divs(tree):
        ...
        for tr in sel_tablerows(day_div):
            tds = tr.getchildren()
            category = self.clean_string(tds[0].text_content())
            menu = self.clean_string(tds[1].text_content())
            ...
            if menu:
                menus.append({'name': menu,
                              'category': category,
                              'language': lang})
        data.append({'date': self.format_date(date), 'dishes': menus})
    return data


class VubScraper(UniScraper):

    def __init__(self):
        self.name = 'VUB Etterbeek'
